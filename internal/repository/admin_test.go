...
func TestReportsBlocklistStats(t *testing.T) {
	...
	require.NoError(t, repo.RecordAudit(ctx, entity.AuditEntry{ActorID: &admin.ID, Action: "room.delete", TargetType: "room", TargetID: "2"}))
	audit, err := repo.ListAudit(ctx, AuditQuery{RoomID: &room.ID, Limit: 10})
	require.NoError(t, err)
	assert.Len(t, audit, 1)
	audit, _ = repo.ListAudit(ctx, AuditQuery{Limit: 10})
	assert.Len(t, audit, 2)
	other := uuid.New()
	audit, _ = repo.ListAudit(ctx, AuditQuery{RoomID: &other, Limit: 10})
	assert.Empty(t, audit)
	// Action prefix, actor and the cursor.
	audit, _ = repo.ListAudit(ctx, AuditQuery{Action: "room.", Limit: 10})
	assert.Len(t, audit, 1)
	audit, _ = repo.ListAudit(ctx, AuditQuery{ActorID: &admin.ID, Limit: 10})
	assert.Len(t, audit, 2)
	audit, _ = repo.ListAudit(ctx, AuditQuery{BeforeID: audit[0].ID, Limit: 10})
	assert.Len(t, audit, 1)
