...
func TestReportsBlocklistStats(t *testing.T) {
	...
	require.NoError(t, repo.CreateReport(ctx, m.ID, alice.ID, entity.ReportNSFW, "nope"))
	assert.ErrorIs(t, repo.CreateReport(ctx, m.ID, alice.ID, entity.ReportOther, ""), ErrConflict)
	require.NoError(t, repo.CreateReport(ctx, m.ID, admin.ID, entity.ReportCopyright, ""))
	require.NoError(t, repo.CreateReport(ctx, m2.ID, alice.ID, entity.ReportOther, ""))
