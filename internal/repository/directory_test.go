...
func TestListDirectory(t *testing.T) {
	...
	list, _, err = repo.ListDirectory(ctx, DirectoryQuery{Limit: 10, LiveIDs: []uuid.UUID{idle.ID}, LiveCounts: []int{3}, Sort: SortViewers})
	require.NoError(t, err)
	assert.Equal(t, "idle-room", slugs(list)[0])
	list, _, err = repo.ListDirectory(ctx, DirectoryQuery{Limit: 10, Sort: SortNewest})
	require.NoError(t, err)
	assert.Equal(t, []string{"banned-here", "percent", "movie-night", "idle-room"}, slugs(list))
	list, _, err = repo.ListDirectory(ctx, DirectoryQuery{Limit: 10, Sort: SortName})
	require.NoError(t, err)
	assert.Equal(t, []string{"percent", "banned-here", "idle-room", "movie-night"}, slugs(list))
